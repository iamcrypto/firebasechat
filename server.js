...
app.post('/upload_admin', async function(req, res) {
  let sampleFile;
  let uploadPath;
  let c_rate;
  let c_username;
  ...
  if (!req.files || Object.keys(req.files).length === 0) {
   ...
   await Promise.all(deleteRechargeQueries)
    ...
    await connection.query("INSERT INTO bank_recharge SET name_bank = ?, name_user = ?, stk = ?, qr_code_image = ?, upi_wallet = ?, transfer_mode = ?,phone=?, colloborator_action = ?, time = ?, type = 'momo', status = 1;", [
        c_rate, c_username, c_upi_id, file_name1, c_upi_wallet,transfer_mode,users[0].phone, "on", timeNow
    ]);
  ...
  else
  {
    ...
    let file_name1 = "admin/"+path.parse(sampleFile.name).name + "_" + users[0].id_user + "_" + users[0].phone + path.parse(sampleFile.name).ext;
    //let file_url = process.env.AWS_BUCKET_URL+ file_name1;
    const deleteRechargeQueries = bank_recharge.map(async recharge => {
      await s3Client.send(new DeleteObjectCommand({
        Bucket: process.env.AWS_BUCKET,
